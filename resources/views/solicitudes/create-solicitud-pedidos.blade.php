                ...
                <form action="{{ route('solicitudes.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <select name="area_solicitante" id="area_solicitante" required class="w-full px-4 py-2 border rounded-lg main-area-select">
                                <option value="">Seleccione área...</option>
                                @foreach($areasBodega as $area)
                                    {{-- CORRECCIÓN: Usamos nombre_area en lugar de imprimir todo el objeto --}}
                        ...
                        <div>
                            ...
                            <select name="centro_costos" id="centro_costos" required class="w-full px-4 py-2 border rounded-lg main-centro-select">
...
// SINCRONIZACIÓN DE CAMPOS PRINCIPALES (ARRIBA)
document.addEventListener('DOMContentLoaded', function() {
    const mainAreaSelect = document.querySelector('.main-area-select');
    const mainCentroSelect = document.querySelector('.main-centro-select');

    if (mainAreaSelect && mainCentroSelect) {
        mainAreaSelect.addEventListener('change', function() {
            syncAreaToCentro(this, mainCentroSelect);
        });

        mainCentroSelect.addEventListener('change', function() {
            syncCentroToArea(this, mainAreaSelect);
        });
    }
    
    // Sincronizar también la primera fila de la tabla
...
// Preparar opciones de Areas una sola vez (usando el array JS ya cargado)
// Nota: Usamos centrosCostosData para sacar los nombres únicos, ya que $areasBodega viene igual
const uniqueAreas = [...new Set(centrosCostosData.map(item => item.nombre_area))].sort();
...
function agregarFila() {
    ...
    uniqueAreas.forEach(areaName => {
        areaOptions += `<option value="${areaName}">${areaName}</option>`;
    });
